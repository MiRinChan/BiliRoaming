...
**Goal:** 新增 `bili_danmaku_purify.js` 脚本，将 DmSegMobile gRPC 响应中的大会员彩色弹幕颜色替换为白色或粉色。

**Architecture:** 独立脚本，从 bili_purify.js 复制 ~300 行 gRPC 基础设施（Base64、gzip、protobuf 助手、unwrap/rewrap）。新增 ~80 行弹幕颜色替换逻辑：遍历 DmSegMobileReply protobuf，定位每个 DanmakuElem 内的 color varint field，逐 field 重建消息以处理 varint 长度变化。

**Tech Stack:** ES5 JavaScript，目标平台为 Loon/Surge/Quantumult X 的 JavaScriptCore 引擎。无 Node.js 依赖。

## Global Constraints

- ES5 语法（兼容 JSC），4 空格缩进，中文注释
- Fail-open: 任何异常 → `$done({})`
- Loon `$response.body` 为 base64 string，Surge 为 `$response.bodyBytes` Uint8Array
- 参数格式兼容 Loon object 和 Surge string
- Plugin argument: `弹幕 = select,"off","white","pink",tag=净化—弹幕`
- Script entry URL path match: `DmSegMobile` via `String.includes()`

---

### Task 1: Create `scripts/bili_danmaku_purify.js`

**Files:**
- Create: `scripts/bili_danmaku_purify.js`

**Interfaces:**
- Produces: `encodeVarint(v)` → `Uint8Array`, `rebuildElemWithColor(elemBytes, targetColor)` → `Uint8Array|null`, `modifyDanmakuColor(buf, targetColor)` → `Uint8Array|null`, `handleDanmakuPurify(targetColor)` → void (calls `$done`)

The complete script (no placeholder — every function has full implementation):

- [ ] **Step 1: Create the script file with header comment and argument parsing**

Write `scripts/bili_danmaku_purify.js`:
...
- [ ] **Step 2: Add the main handler and color modification functions**

Append to `scripts/bili_danmaku_purify.js`:
...
- [ ] **Step 3: Append the IO layer and gRPC infrastructure (copied from bili_purify.js)**
...
- [ ] **Step 4: Verify file structure — run `wc -l`**

```bash
wc -l scripts/bili_danmaku_purify.js
```
...
- Produces: script URL mapping for DmSegMobile with argument `[{弹幕}]`
...
via DmSegMobile gRPC protobuf manipulation.
...
After implementation, verify with the captured sample data:

```bash
# Decode the sample DmSegMobile response, apply white mode, count remaining colors
node -e "
// Use the manual varint encoder to verify
function encodeVarint(v) {
    var r = [];
...
console.log('0xFFFFFF varint len:', encodeVarint(0xFFFFFF).length, '(expect 4)');
console.log('0xFFAEC9 varint len:', encodeVarint(0xFFAEC9).length, '(expect 4)');
// Verify both targets are 4 bytes — the logic assumes this for buf expansion
"
```
